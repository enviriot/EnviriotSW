...
namespace X13.WebUI.Host {
  ...
  // rendered as a third root alongside "State" and "Children" inside the same
  // document (see InspectorStateViewProvider/InspectorChildrenViewProvider). One
  // ManifestTreeController is created lazily per distinct root vid, the first time
  // it's expanded, and disposed explicitly via Close(vid) when the frontend
  // navigates away from that document.
  internal sealed class InspectorManifestViewProvider : ViewProviderBase {
    private const string ViewName = "inspmanifest";

    private readonly Action<JSC.JSObject> _send;
    private readonly ViewTargetRegistry _targets;
    private readonly object _gate = new object();
    private readonly Dictionary<string, ManifestTreeController> _roots = new Dictionary<string, ManifestTreeController>(StringComparer.Ordinal);

    public InspectorManifestViewProvider(Action<JSC.JSObject> send, ViewTargetRegistry targets) {
      _send = send;
      _targets = targets;
    }
    ...
    public override ViewOpResult Expand(string vid, bool expand) {
      ManifestTreeController controller = GetOrCreateOwner(vid);
      if(controller == null) {
    ...
    public override ViewOpResult Commit(string vid, JSC.JSValue value) {
      ManifestTreeController controller = FindOwner(vid);
      if(controller == null) {
    ...
    public override void Dispose() {
      lock(_gate) {
        foreach(ManifestTreeController controller in _roots.Values) controller.Dispose();
        _roots.Clear();
      }
    }

    private ManifestTreeController GetOrCreateOwner(string vid) {
      lock(_gate) {
        ManifestTreeController existing = FindOwnerLocked(vid);
        ...
        ManifestTreeController controller = new ManifestTreeController(_send, _targets, rootTopic, ViewName);
    ...
    private ManifestTreeController FindOwner(string vid) {
      lock(_gate) {
        return FindOwnerLocked(vid);
      }
    }

    private ManifestTreeController FindOwnerLocked(string vid) {
    ...
    // ManifestTreeController ever owns shares the *one* root topic path, differing
    // only in field path. IsDescendant walks parent vids via dotted field-path
    // removal, which also happens to match when one topic's path is textually a
    // path-prefix of another *unrelated* topic's path (e.g. root "/Test/X" vs a
    // request for its real child topic "/Test/X/Y", both with empty field path) -
    // that's a different topic's own inspmanifest document, not a field of this one,
    // and must not be routed here (confirmed live: a stray commit landed on the wrong
    // topic's manifest before this fix - see StateTreeController's identical fix).
