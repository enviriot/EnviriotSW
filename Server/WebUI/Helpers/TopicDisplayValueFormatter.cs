...
namespace X13.WebUI.Helpers {
  internal static class TopicDisplayValueFormatter {
    ...
    public static string Format(JSC.JSValue state) {
      if(state == null || !state.Defined || state.IsNull) return string.Empty;
      if(state.ValueType == JSC.JSValueType.Object) {
        if(state.Value == null) return string.Empty;
    ...
    private static string FormatArrayItem(JSC.JSValue value) {
      ...
      if(value == null || !value.Defined || value.IsNull) return "null";
      if(value.ValueType == JSC.JSValueType.String) return "\"" + TrimDisplay(value.Value as string) + "\"";
    ...
    private static bool IsScalarDisplayValue(JSC.JSValue value) {
      ...
      return value.ValueType != JSC.JSValueType.Object;
    }
