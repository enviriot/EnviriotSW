...
namespace X13.WebUI.Helpers {
  internal static class VidHelper {
    ...
    public static bool IsDescendant(string parentVid, string candidateVid) {
      ...
      // Cheap necessary condition: every parent vid is a string prefix of its child
      // ("view#/a/b"->"view#/a", "view#/a#f.g"->"view#/a#f", "view#/a#f"->"view#/a",
      // "view#/a"->"view#/"), so a non-prefix can never be an ancestor. Only a pre-filter -
      // the walk below still decides, so boundary cases keep their existing behaviour - but
      // it turns the common "unrelated key" case from a 256-level climb (five Split('#')
      // per level) into one comparison. RemoveSubtree calls this once per registered target.
      if(!candidateVid.StartsWith(parentVid, StringComparison.Ordinal)) return false;
